...
const CATEGORY_EMOJIS: Record<string, string> = {
  'Digestive Health': '🫁', 'Weight Management': '⚖️', 'Pregnancy': '🤰',
  'Metabolic Health': '🩸', 'Kidney Health': '🫘', 'Liver Health': '🫀',
  'Heart Health': '❤️', 'Thyroid Health': '🦋', 'Skin Health': '✨',
  'Bone & Joint Health': '🦴', 'Respiratory Health': '🫁', 'Reproductive Health': '🌸',
  'Blood Health': '🩸', 'Cancer Support': '🎗️', 'Child Health': '👶',
  'Seasonal Diet': '🌿', 'Ayurvedic Constitution': '🕉️', 'Mental Health': '🧠',
  'Eye Health': '👁️', 'GI Disorders': '🔥', 'Allergy Care': '🤧',
  'Complex Conditions': '💊', 'Neurological & Surgical': '🧬', 'Immune & Chronic': '🛡️',
  'Specific Conditions': '🎯', 'General Health': '🌿',
};

const CATEGORY_ICONS: Record<string, React.ComponentType<any>> = {
  'Digestive Health': Activity, 'Weight Management': Activity, 'Pregnancy': Heart,
  'Metabolic Health': Droplet, 'Kidney Health': Droplet, 'Liver Health': Shield,
  'Heart Health': Heart, 'Thyroid Health': Activity, 'Skin Health': Sparkles,
  'Bone & Joint Health': Bone, 'Respiratory Health': Wind, 'Reproductive Health': Flower2,
  'Blood Health': Droplet, 'Cancer Support': Shield, 'Child Health': Baby,
  'Seasonal Diet': Sun, 'Ayurvedic Constitution': Leaf, 'Mental Health': Brain,
  'Eye Health': Eye, 'GI Disorders': Activity, 'Allergy Care': Shield,
  'Complex Conditions': Pill, 'Neurological & Surgical': Brain, 'Immune & Chronic': Shield,
  'Specific Conditions': Stethoscope, 'General Health': Leaf,
};

const CATEGORY_GRADIENTS: Record<string, string> = {
  'Digestive Health': 'from-amber-500 to-orange-600', 'Weight Management': 'from-green-500 to-emerald-600',
  'Pregnancy': 'from-pink-500 to-rose-600', 'Metabolic Health': 'from-red-500 to-rose-600',
  'Kidney Health': 'from-blue-500 to-indigo-600', 'Liver Health': 'from-amber-600 to-yellow-700',
  'Heart Health': 'from-red-500 to-pink-600', 'Thyroid Health': 'from-purple-500 to-violet-600',
  'Skin Health': 'from-pink-400 to-fuchsia-600', 'Bone & Joint Health': 'from-stone-500 to-stone-700',
  'Respiratory Health': 'from-sky-500 to-cyan-600', 'Reproductive Health': 'from-fuchsia-500 to-purple-600',
  'Blood Health': 'from-red-600 to-red-800', 'Cancer Support': 'from-violet-500 to-purple-700',
  'Child Health': 'from-yellow-400 to-amber-500', 'Seasonal Diet': 'from-lime-500 to-green-600',
  'Ayurvedic Constitution': 'from-teal-500 to-emerald-600', 'Mental Health': 'from-indigo-500 to-blue-600',
  'Eye Health': 'from-sky-400 to-blue-500', 'GI Disorders': 'from-orange-500 to-red-500',
  'Allergy Care': 'from-cyan-500 to-teal-600', 'Complex Conditions': 'from-slate-500 to-zinc-700',
  'Neurological & Surgical': 'from-violet-600 to-indigo-700', 'Immune & Chronic': 'from-emerald-600 to-green-700',
  'Specific Conditions': 'from-rose-500 to-red-600', 'General Health': 'from-teal-500 to-emerald-600',
};
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 text-center">
          ...
          <div className={`max-w-2xl mx-auto transition-all duration-700 delay-400 ${heroObserver.hasBeenVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            <div className="relative">
              ...
              />
              {searchQuery && (
                <button onClick={() => setSearchQuery('')} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-800 transition-colors">
                  <X className="w-5 h-5" />
                </button>
              )}
            </div>
      ...
      <section className="relative -mt-8 z-10 max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex gap-3 overflow-x-auto pb-4 hide-scrollbar -mx-4 px-4">
          ...
          >
            ...
            All Plans
            <span className={`text-xs px-2 py-0.5 rounded-full ${selectedCategory === 'All' ? 'bg-white/20' : 'bg-gray-100'}`}>{allCharts.length}</span>
          </button>
          {categories.slice(0, 10).map(cat => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full ${isSelected ? 'bg-white/20' : 'bg-gray-100'}`}>{categoryCounts[cat] || 0}</span>
              </button>
            );
          })}
        </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 mt-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <button onClick={() => setShowFilters(!showFilters)} className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-semibold text-sm transition-all duration-300 border-2 ${showFilters ? 'bg-ayur-green text-white border-ayur-green' : 'bg-white text-gray-800 border-gray-200 hover:border-ayur-green/30'}`}>
              <Filter className="w-4 h-4" />
              Filters
              <ChevronDown className={`w-4 h-4 transition-transform ${showFilters ? 'rotate-180' : ''}`} />
            </button>
            {hasActiveFilters && (
              <button onClick={clearFilters} className="flex items-center gap-1 px-3 py-2 rounded-xl text-sm font-medium text-red-500 hover:bg-red-50 transition-colors">
                <X className="w-4 h-4" />
                Clear all
              </button>
            )}
          ...
          <div className="flex items-center gap-3">
            ...
            <select value={sortBy} onChange={(e) => setSortBy(e.target.value as any)} className="px-3 py-2 rounded-xl border-2 border-gray-200 text-sm font-medium text-gray-800 bg-white focus:border-ayur-green focus:outline-none transition-colors">
          ...
          <div className="mt-4 p-5 bg-white rounded-2xl border border-gray-200 shadow-sm animate-fadeIn">
            ...
            <div className="flex flex-wrap gap-2">
              {PRAKRITI_OPTIONS.map(prakriti => (
                <button key={prakriti} onClick={() => setSelectedPrakriti(prakriti)} className={`px-4 py-2 rounded-xl text-sm font-medium transition-all duration-300 ${selectedPrakriti === prakriti ? 'bg-ayur-green text-white shadow-md' : 'bg-gray-50 text-gray-500 hover:bg-ayur-green-light hover:text-ayur-green'}`}>
                  {prakriti}
                </button>
              ))}
            </div>
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 mt-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {featuredCharts.map((chart, idx) => (
              <DietChartCard key={chart.id} title={chart.title} slug={chart.slug} category={chart.category} description={chart.description} image={chart.image} foodGroups={chart.foodGroups} foodsToConsume={chart.foodsToConsume} foodsToAvoid={chart.foodsToAvoid} lifestyleTips={chart.lifestyleTips} featured={true} index={idx} />
            ))}
          </div>
        </section>
      )}
      ...
      <section ref={gridObserver.ref} className="max-w-7xl mx-auto px-4 sm:px-6 mt-10 mb-12">
        <div className="flex items-center gap-3 mb-6">
          <h2 className="font-serif text-2xl md:text-3xl font-bold text-gray-800">
            {searchQuery ? `Results for "${searchQuery}"` : selectedCategory !== 'All' ? selectedCategory : selectedPrakriti !== 'All' ? `${selectedPrakriti} Diet Charts` : 'All Diet Charts'}
          </h2>
        </div>
        ...
        {filteredCharts.length > 0 ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-500 mb-6 max-w-md mx-auto">Try adjusting your search or filters to find the perfect diet plan.</p>
            <button onClick={clearFilters} className="px-6 py-3 bg-ayur-green text-white rounded-2xl font-semibold hover:bg-ayur-green-dark transition-colors">Clear Filters</button>
          </div>
        )}
      </section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              ...
              <div key={i} className={`text-center transition-all duration-700 ${statsObserver.hasBeenVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{ transitionDelay: `${i * 150}ms` }}>
                ...
                <div className="text-3xl md:text-4xl font-bold text-white mb-1">{stat.value}{stat.suffix}</div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-16 md:py-20">
        <div className="bg-gradient-to-br from-ayur-accent/10 via-amber-50 to-ayur-accent/5 rounded-3xl p-8 md:p-12 border border-ayur-accent/20 text-center relative overflow-hidden">
          ...
          <div className="relative z-10">
            <p className="font-serif text-xl md:text-2xl text-gray-800 italic mb-4 max-w-3xl mx-auto leading-relaxed">"When diet is wrong, medicine is of no use. When diet is correct, medicine is of no need."</p>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a href="#/booking" className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-ayur-green text-white rounded-2xl font-bold hover:bg-ayur-green-dark transition-all shadow-lg hover:shadow-xl">
                <ChefHat className="w-5 h-5" /> Consult a Dietician
              </a>
              <a href="#/tools" className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-white text-ayur-green border-2 border-ayur-green/20 rounded-2xl font-bold hover:border-ayur-green transition-all">
                <Activity className="w-5 h-5" /> Know Your Prakriti
              </a>
            </div>
          </div>
        </div>
